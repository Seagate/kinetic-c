...
LDFLAGS += -lm -L${OPENSSL_LIBPATH} -lcrypto -lssl -lpthread -ljson-c
NUM_SIMS ?= 2
...
PROTOBUFC_LIB ?= $(VENDOR)/protobuf-c/libprotobuf-c.so
SOCKET99_LIB ?= $(VENDOR)/socket99/libsocket99.a

LDFLAGS += ${PROTOBUFC_LIB} ${SOCKET99_LIB}
...
BUILD_JSONC := $(shell [ -e ${JSONC_INCPATH}/Makefile ] && echo 1 || echo 0)
...
libsocket99.a: ${SOCKET99_INCPATH}/*.[ch]
	if [ -f ${SOCKET99_INCPATH}/Makefile ]; then \
		cd ${SOCKET99_INCPATH} && make all; \
	fi;

libprotobuf-c.so:
	if [ -f ${PROTOBUFC_INCPATH}/autogen.sh ]; then  \
		cd ${PROTOBUFC_INCPATH} && \
		sh autogen.sh  && \
		./configure && make all; \
	fi;
...
$(KINETIC_LIB): $(KINETIC_LIB_OTHER_DEPS) $(LIB_OBJS) $(JSONC_LIB)
	...
	@echo --------------------------------------------------------------------------------
	ar -rcs $@ $(LIB_OBJS) ${SOCKET99_LIB} ${PROTOBUFC_LIB}
	ar -t $@
...
SYSTEST_LDFLAGS += -lm $(KINETIC_LIB) -L${OUT_DIR} -L${OPENSSL_LIBPATH} -lssl -lcrypto -lpthread -ljson-c
...
$(SYSTEST_OUT)/run_%: $(SYSTEST_SRC)/%.c $(SYSTEST_OUT)/%_runner.c $(KINETIC_LIB)
	...
	$(CC) -o $@ $< $(word 2,$^) ./test/support/system_test_fixture.c ./test/support/system_test_kv_generate.c $(UNITY_SRC) $(SYSTEST_CFLAGS) $(LIB_INCS) -I$(UNITY_INC) -I./test/support  \
		$(SYSTEST_LDFLAGS) $(KINETIC_LIB) ${SOCKET99_LIB} ${PROTOBUFC_LIB}
...
UTIL_LDFLAGS += -lm $(KINETIC_LIB) -L${OUT_DIR} -L${OPENSSL_LIBPATH} -lssl -lcrypto -lpthread  ${JSONC_LIB} ${SOCKET99_LIB} ${PROTOBUFC_LIB}
